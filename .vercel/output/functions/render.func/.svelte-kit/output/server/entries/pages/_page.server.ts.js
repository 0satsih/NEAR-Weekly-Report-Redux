import { Flipside } from "@flipsidecrypto/sdk/dist/src/index.js";
import { e as error } from "../../chunks/index2.js";
const FLIPSIDE_KEY = "1f5ac839-b5bd-404a-93ff-8f06f84e3a49";
const load = async () => {
  const flipside = new Flipside(FLIPSIDE_KEY, "https://node-api.flipsidecrypto.com");
  const daily_gas_7days = {
    sql: `
    SELECT
...
ORDER BY
  ...
  const daily_active_users_7days = {
    sql: `
    SELECT
  date_trunc('day', block_timestamp) as date,
...
ORDER BY
  ...
  const daily_active_users_30days = {
    sql: `
    SELECT
  date_trunc('day', block_timestamp) as date,
  count(distinct tx_signer) as users
...
ORDER BY
  ...
  const daily_transactions_7days = {
    sql: `
    with
...
WHERE
  ...
  const daily_transactions_30days = {
    sql: `
    with
  near_transactions as (
    SELECT
...
WHERE
  ...
  const daily_transaction_fee_7days = {
    sql: `
    with
  daily_near_price as (
    SELECT
...
ORDER BY
  ...
  const daily_transaction_fee_30days = {
    sql: `
    with
  daily_near_price as (
    SELECT
...
ORDER BY
  ...
  const daily_active_contracts_7days = {
    sql: `
    with
  contracts as (
    SELECT
...
ORDER BY
  ...
  const daily_active_contracts_30days = {
    sql: `
    with
  contracts as (
    SELECT
...
ORDER BY
  ...
  const daily_new_contracts_7days = {
    sql: `
    with
  contracts_first as (
    SELECT
...
ORDER BY
  ...
  const daily_new_contracts_30days = {
    sql: `
    with
  contracts_first as (
    SELECT
